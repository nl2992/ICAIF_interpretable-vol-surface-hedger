...
```bash
# 1. Synthetic study + real SPY/QQQ study + figures (end-to-end)
make reproduce

# 2. (or) fast synthetic-only smoke run
make run-fast

# 3. (or) real options study only
python scripts/run_real_data.py
...
**Exact reproduction.** Tested with Python 3.11. The committed `reports_real/` tables and `paper/` figures are the exact published numbers; the pipeline is deterministic under the seeds fixed in `configs/`. The raw OptionsDX chains are not committed (~3.8 GB, gitignored) — for the exact real-data run, download them from [OptionsDX](https://www.optionsdx.com/) into `data/raw/{spy,qqq}/` as described above; the synthetic study needs no external data.
