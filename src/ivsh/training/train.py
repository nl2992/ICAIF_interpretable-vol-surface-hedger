...
def _policy_pnl(policy, bank: EpisodeBank, scaler: Standardizer, smooth: bool):
...
def fit_policy(
    ...
    def obj_and_grad(z: np.ndarray):
        ...
        eta = z[n]
        holdings_flat, cache = policy.forward(x_tr)
        holdings = holdings_flat.reshape(E, L, -1)
    ...
    def callback(z):
        if val_bank is None:
            return
        policy.set_flat_params(z[:n])
        pnl, _ = _policy_pnl(policy, val_bank, scaler, smooth=False)
    ...
    z_final = best["z"] if (val_bank is not None and np.isfinite(best["val"])) else res.x
    policy.set_flat_params(z_final[:n])
    pnl, _ = _policy_pnl(policy, train_bank, scaler, smooth=False)
    return policy, {
