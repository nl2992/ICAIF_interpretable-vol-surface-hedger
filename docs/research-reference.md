...
This page retains the original README content with updated punctuation, including its detailed methods, commands,
result tables and claims-to-evidence mapping. It records multiple experiment
configurations and earlier summaries; numerical differences between those summaries
are retained here. For the current overview, use the main README and its links to
specific committed tables.
...
**Prerequisites.** Python 3.11. For the exact pinned environment use conda (`conda env create -f environment.yml && conda activate ivsh`) or pip:
```bash
make install
```
...
prototype multiseed (`multiseed_cvar.csv`). Both are correct because they are different configs. All numbers
