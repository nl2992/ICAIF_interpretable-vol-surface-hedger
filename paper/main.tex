...
Our answer, on a controlled synthetic market and on \emph{two} real option
markets (SPY 2010--2023 and QQQ 2012--2023), is a qualified yes
(Figure~\ref{fig:teaser}): the interpretable hedger \emph{matches} delta--vega on the
tail across both markets while \emph{dominating} every learned black-box and deep-RL
hedger, which overfit and blow up out-of-sample. We build on prototype-based interpretable
hedging~\cite{protohedge2025}---which makes each decision a similarity-weighted
blend of learned per-prototype actions---and extend it in four substantive
...
mean utility; we validate on \emph{real, non-martingale} option data via an
anchored-residual formulation; and we show that making this robustness
\emph{transfer across markets} requires a deliberate, validation-only
model-selection recipe---naive selection overfits under regime shift.
...
\item A \textbf{model-selection recipe for robustness under regime shift}: we show
that a naive validation-selected residual \emph{adds} tail risk on a second market,
diagnose the mechanism, and demonstrate that a tail-weighted objective---chosen by a
pre-registered, validation-only search and confirmed once on each market's held-out
test---restores cross-market parity with delta--vega (Section~\ref{sec:fix}).
